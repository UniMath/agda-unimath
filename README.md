...
Concepts are first-class citizens the `agda-unimath` library. An index of the concepts formalized in `agda-unimath` is formed by the list of files in the library, and the indexing terms are the file names. Since we want to help humans looking for their favorite topic to find it easily, all the files in our library introduce one concept, or are about one named theorem or about one narrow topic. The file names describe that concept, theorem, or topic in a concise and natural manner.
