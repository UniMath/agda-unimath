...
AGDAHTMLFLAGS ?= --html --html-highlight=auto --html-dir=$(AGDA_HTML_DIR) --css=$(WEBSITE_CSS_DIR)/Agda.css --only-scope-checking
...
profile-module:
	...
	@$(AGDA) $(PROFILE_MODULE_AGDA_ARGS) $(SOURCE_DIR)/$(MODULE_DIR).lagda.md 2>&1 | tee ./$(PROFILING_OUTPUT_FILE)
	@# Check for additional modules being typechecked by looking for any indented "Checking" line
	@if grep -E "^\s+Checking " ./$(PROFILING_OUTPUT_FILE) > /dev/null; then \
...
# Use bash if instead of ifeq, because we want to change SKIPAGDA with rules
	...
	else python3 ./$(SCRIPTS_DIR)/generate_noagda_html.py ./$(AGDA_HTML_DIR)/; \
	fi
...
website-prepare: agda-html ./SUMMARY.md ./CONTRIBUTORS.md ./MAINTAINERS.md \
	...
	@cp $(METAFILES) ./$(AGDA_HTML_DIR)/
	@mkdir -p ./$(AGDA_HTML_DIR)/website
	@cp -r ./$(WEBSITE_IMAGES_DIR) ./$(AGDA_HTML_DIR)/website/
	@cp -r ./$(WEBSITE_CSS_DIR) ./$(AGDA_HTML_DIR)/website/
	@cp -r ./$(WEBSITE_JS_DIR) ./$(AGDA_HTML_DIR)/website/
	@cp -r ./$(TABLES_DIR) ./$(AGDA_HTML_DIR)
...
.PHONY: graph
graph: $(AGDA_HTML_DIR)/dependency.dot
