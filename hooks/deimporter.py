...
if __name__ == "__main__":
    ...
    agda_files = filter(lambda f: utils.isAgdaFile(pathlib.Path(
        f)) and os.path.dirname(f) != "src", get_files("src"))
...
if __name__ == "__main__":
    foundation_files = filter(lambda f: utils.isAgdaFile(pathlib.Path(f)), itertools.chain(
        get_files("src/foundation"), get_files("src/foundation-core")))
        ...
        with open(agda_file, 'r', encoding='UTF-8') as file:
            ...
            for statement in namespaces["foundation"]:
                ...
                if (subprocess.call("agda " + agda_file, shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL) != 0):
                    ...
                    print(
                        f"  the module '{statement}' could be imported from core instead")
