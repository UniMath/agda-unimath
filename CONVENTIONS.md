...
```
5. Immediately after the header, there should be an Agda block code within a `details` block with a `summary` block containing the text *Imports* as shown
in the template below. The Agda block should include the module declaration and all module imports. In particular, there should be no further imports later on in the file. The module names should always be in lowercase. By default, Agda imports will be hidden but they can be revealed by clicking on the Imports link.
