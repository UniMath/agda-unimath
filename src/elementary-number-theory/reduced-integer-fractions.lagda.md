...
A
{{#concept "reduced fraction" WDID=Q263865 WD="irreducible fraction" Agda=is-reduced-fraction-ℤ}}
is a [fraction](elementary-number-theory.integer-fractions.md) in which its
...
### An integer fraction is equal, not just similar, to its reduced fraction times a diagonal fraction

```agda
diagonal-gcd-fraction-ℤ : fraction-ℤ → fraction-ℤ
diagonal-gcd-fraction-ℤ (p , q , pos-q) =
  in-diagonal-fraction-ℤ
    ( gcd-ℤ p q , is-positive-gcd-is-positive-right-ℤ p q pos-q)

abstract
  eq-mul-reduce-diagonal-fraction-ℤ :
    (x : fraction-ℤ) →
    x ＝ reduce-fraction-ℤ x *fraction-ℤ diagonal-gcd-fraction-ℤ x
  eq-mul-reduce-diagonal-fraction-ℤ x =
    eq-fraction-ℤ _ _
      ( inv (pr2 (reduce-numerator-fraction-ℤ x)))
      ( inv (pr2 (reduce-denominator-fraction-ℤ x)))
```
