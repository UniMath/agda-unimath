...
decidable-subtype-le-ℕ n m =
  (m <-ℕ n , is-prop-le-ℕ m n , is-decidable-le-ℕ m n)
```
...
```agda
abstract
  le-is-nonzero-ℕ : (n : ℕ) → is-nonzero-ℕ n → zero-ℕ <-ℕ n
...
```agda
abstract
  transitive-le-ℕ : is-transitive _<-ℕ_
...
### The strictly preordered set of natural numbers

```agda
ℕ-Strictly-Preordered-Set : Strictly-Preordered-Set lzero lzero
pr1 ℕ-Strictly-Preordered-Set = ℕ-Set
pr2 ℕ-Strictly-Preordered-Set =
  le-ℕ-Prop , irreflexive-le-ℕ , λ n m l I J → transitive-le-ℕ n m l I J
```
...
abstract
  ...
  le-subtraction-ℕ zero-ℕ m l q p =
    tr (zero-ℕ <-ℕ_) p (le-is-nonzero-ℕ l q)
...
```agda
abstract
  preserves-strict-order-succ-ℕ :
    (m n : ℕ) → m <-ℕ n → m <-ℕ succ-ℕ n
  preserves-strict-order-succ-ℕ m n H =
...
```agda
abstract
  le-not-leq-ℕ : (m n : ℕ) → ¬ (n ≤-ℕ m) → m <-ℕ n
...
abstract
  preserves-strict-order-mul-ℕ :
    (a b c d : ℕ) → a <-ℕ b → c <-ℕ d → a *ℕ c <-ℕ b *ℕ d
  preserves-strict-order-mul-ℕ zero-ℕ (succ-ℕ b) zero-ℕ (succ-ℕ d) H K =
    star
  preserves-strict-order-mul-ℕ zero-ℕ (succ-ℕ b) (succ-ℕ c) (succ-ℕ d) H K =
    star
