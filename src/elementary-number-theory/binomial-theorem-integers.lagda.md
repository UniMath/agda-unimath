...
```agda
binomial-sum-ℤ : (n : ℕ) (f : functional-tuple ℤ (succ-ℕ n)) → ℤ
...
binomial-sum-one-element-ℤ :
  (f : functional-tuple ℤ 1) → binomial-sum-ℤ 0 f ＝ head-functional-tuple 0 f
binomial-sum-one-element-ℤ =
  binomial-sum-one-element-Commutative-Ring ℤ-Commutative-Ring

binomial-sum-two-elements-ℤ :
  (f : functional-tuple ℤ 2) →
...
```agda
htpy-binomial-sum-ℤ :
  (n : ℕ) {f g : functional-tuple ℤ (succ-ℕ n)} →
...
left-distributive-mul-binomial-sum-ℤ :
  (n : ℕ) (x : ℤ) (f : functional-tuple ℤ (succ-ℕ n)) →
...
right-distributive-mul-binomial-sum-ℤ :
  (n : ℕ) (f : functional-tuple ℤ (succ-ℕ n)) (x : ℤ) →
