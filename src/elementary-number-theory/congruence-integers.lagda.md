...
### If an integer is congruent to its successor or predecessor, then the modulus is a unit

```agda
abstract
  is-unit-modulus-cong-succ-ℤ : (k x : ℤ) → cong-ℤ k x (succ-ℤ x) → is-unit-ℤ k
  is-unit-modulus-cong-succ-ℤ k x (y , p) =
    is-unit-div-one-ℤ k
      ( neg-ℤ y ,
  ...
  is-unit-modulus-cong-pred-ℤ : (k x : ℤ) → cong-ℤ k x (pred-ℤ x) → is-unit-ℤ k
  is-unit-modulus-cong-pred-ℤ k x (y , p) =
    is-unit-div-one-ℤ k
      ( y ,
        ( p) ∙
...
### Congruence modulo `k` is an equivalence relation
