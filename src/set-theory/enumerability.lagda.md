# Enumerability of types

```agda
module set-theory.enumerability where
```
...
`α`-[enumeration](set-theory.enumerations.md) of `X`, `α ↠ Maybe X`.

If `α` and `X` are [discrete types](foundation.discrete-types.md), then `X` is
`α`-enumerable if and only if `X` is a
...
### The type of α-enumerable types

```agda
Enumerable-Type : {l1 : Level} (l2 : Level) (α : UU l1) → UU (l1 ⊔ lsuc l2)
Enumerable-Type l2 α = Σ (UU l2) (is-enumerable α)
```
