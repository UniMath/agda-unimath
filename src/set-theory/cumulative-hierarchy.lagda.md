    ...
    ( Σ ({A : UU l} → (A → V) → V)
      ( λ V-set →
        ( {A B : UU l} (f : A → V) (g : B → V) →
          (type-trunc-Prop (lift f g) × type-trunc-Prop (lift g f)) → V-set f ＝ V-set g)))
...
```agda
  prop-ind-principle-cumulative-hierarchy : {l2 : Level}
    ( P : type-pseudo-cumulative-hierarchy V → UU l2) →
  ...
  compute-prop-ind-principle-cumulative-hierarchy : {l2 : Level}
    ( P : type-pseudo-cumulative-hierarchy V → UU l2) →
    ...
    { A : UU l1} (f : A → type-pseudo-cumulative-hierarchy V)
    ( IH : (a : A) → P (f a)) →
    prop-ind-principle-cumulative-hierarchy P σ ρ (set-pseudo-cumulative-hierarchy V f)
      ＝ ρ f IH
...
```agda
  recursion-principle-cumulative-hierarchy : {l2 : Level}
    ( X : UU l2) (σ : is-set X)
  ...
  compute-recursion-principle-cumulative-hierarchy : {l2 : Level}
    ( X : UU l2) (σ : is-set X)
    ( ρ : {A : UU l1} → (A → type-pseudo-cumulative-hierarchy V) → (A → X) → X)
    ( τ : {A B : UU l1} (f : A → type-pseudo-cumulative-hierarchy V)
      ( g : B → type-pseudo-cumulative-hierarchy V)
...
```agda
  prop-recursion-principle-cumulative-hierarchy : {l2 : Level}
    ( ρ : {A : UU l1} → (A → type-pseudo-cumulative-hierarchy V) →
          (A → Prop l2) → Prop l2)
    ( τ : {A B : UU l1} (f : A → type-pseudo-cumulative-hierarchy V)
      ( g : B → type-pseudo-cumulative-hierarchy V)
  ...
  prop-recursion-principle-cumulative-hierarchy {l2} ρ τ =
    ...
    where
      τ' : {A B : UU l1} (f : A → type-pseudo-cumulative-hierarchy V)
        ...
        ( (a : A) → ∃ B (λ b → Σ (f a ＝ g b) (λ _ → IH₁ a ＝ IH₂ b))) →
        ( (b : B) → ∃ A (λ a → Σ (g b ＝ f a) (λ _ → IH₂ b ＝ IH₁ a))) →
        ρ f IH₁ ＝ ρ g IH₂
      τ' f g (e₁ , e₂) IH₁ IH₂ hIH₁ hIH₂ =
        eq-iff (τ f g e₂ IH₁ IH₂ hIH₁) (τ g f e₁ IH₂ IH₁ hIH₂)

  compute-prop-recursion-principle-cumulative-hierarchy : {l2 : Level}
    ( ρ : {A : UU l1} → (A → type-pseudo-cumulative-hierarchy V) →
          (A → Prop l2) → Prop l2)
    ( τ : {A B : UU l1} (f : A → type-pseudo-cumulative-hierarchy V)
      ( g : B → type-pseudo-cumulative-hierarchy V)
      ...
      ( (a : A) → ∃ B (λ b → Σ (f a ＝ g b)
        ( λ _ → IH₁ a ＝ IH₂ b))) →
      type-Prop (ρ f IH₁) → type-Prop (ρ g IH₂)) →
    {A : UU l1} → (f : A → type-pseudo-cumulative-hierarchy V) → (IH : A → Prop l2) →
...
```agda
  simple-prop-recursion-principle-cumulative-hierarchy : {l2 : Level}
...
```agda
  ...
  ∈-cumulative-hierarchy-Prop x =
    ...
    where
      e : {A B : UU l1} (f : A → type-pseudo-cumulative-hierarchy V)
        ( g : B → type-pseudo-cumulative-hierarchy V) →
