...
module _
  ...
  abstract
    Eq-map-list-map-tuple-list :
      (l : list A) →
      Eq-list
        ( list-tuple (length-list l) (map-tuple f (tuple-list l)))
        ( map-list f l)
    Eq-map-list-map-tuple-list nil = raise-star
    Eq-map-list-map-tuple-list (cons x xs) =
      ( refl , Eq-map-list-map-tuple-list xs)
    ...
    map-list-map-tuple-list l =
      eq-Eq-list _ _ (Eq-map-list-map-tuple-list l)
...
```agda
abstract
  preserves-id-map-list :
    {l1 : Level} {A : UU l1} →
    map-list (id {A = A}) ~ id
  preserves-id-map-list nil = refl
  preserves-id-map-list (cons a x) =
    ap (cons a) (preserves-id-map-list x)

  preserves-comp-map-list :
    {l1 l2 l3 : Level} {A : UU l1} {B : UU l2} {C : UU l3} →
    (f : A → B) (g : B → C) →
    map-list (g ∘ f) ~ (map-list g ∘ map-list f)
  preserves-comp-map-list f g nil = refl
  preserves-comp-map-list f g (cons a x) =
    ap (cons (g (f a))) (preserves-comp-map-list f g x)
```
