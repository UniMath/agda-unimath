...
module _
  ...
  ( R : (n : ℕ) (X : UU-Fin l1 n) → Eq-Relation l3 (D n X))
  ( is-decidable-R :
    (n : ℕ) → leq-ℕ 2 n → (X : UU-Fin l1 n)
    (a b : D n X) → is-decidable (sim-Eq-Relation (R n X) a b))
    ...
    preserves-R-invertible-action-D-equiv n X X' =
      Ind-univalent-action-equiv (mere-equiv-Prop (Fin n)) (D n) X
        ( λ Y f →
          ( a a' : D n X) →
          ( sim-Eq-Relation (R n X) a a' ↔
            sim-Eq-Relation (R n Y) (map-equiv f a) (map-equiv f a')))
        ( λ a a' → id , id)
        ( X')
...
module _
  ...
  private
    equiv-Q-equivalence-class :
      (n : ℕ) (X : UU-Fin l1 n) →
      type-UU-Fin 2 (Q n X) ≃
      equivalence-class (Id-Eq-Relation (set-UU-Fin 2 (Q n X)))
    equiv-Q-equivalence-class n X =
      equiv-uniqueness-set-quotient
        ( Id-Eq-Relation (set-UU-Fin 2 (Q n X)))
        ( set-UU-Fin 2 (Q n X))
        ( id-reflecting-map-Id-Eq-Relation (set-UU-Fin 2 (Q n X)))
        ( is-set-quotient-id-Id-Eq-Relation (set-UU-Fin 2 (Q n X)))
        ( equivalence-class-Set (Id-Eq-Relation (set-UU-Fin 2 (Q n X))))
    ...
    equiv-fin-2-equivalence-class :
      ...
      Fin 2 ≃ equivalence-class (Id-Eq-Relation (set-UU-Fin 2 (Q n X)))
    equiv-fin-2-equivalence-class n X H h =
      tr
        ( λ Y →
          Fin 2 ≃ equivalence-class (Id-Eq-Relation (set-UU-Fin 2 (Q n Y))))
        ( eq-pair-Σ
  ...
  delooping-sign =
    quotient-delooping-sign
      ...
      ( λ n X → Id-Eq-Relation (set-UU-Fin 2 (Q n X)))
