# Free algebras

```agda
{-# OPTIONS --lossy-unification #-}

module universal-algebra.free-algebras where
```

<details><summary>Imports</summary>

```agda
open import elementary-number-theory.natural-numbers

open import foundation.action-on-homotopies-functions
open import foundation.action-on-identifications-functions
open import foundation.binary-relations
open import foundation.dependent-pair-types
open import foundation.equivalence-relations
open import foundation.equivalences
open import foundation.function-extensionality
open import foundation.function-types
open import foundation.functoriality-propositional-truncation
open import foundation.functoriality-set-quotients
open import foundation.homotopies
open import foundation.identity-types
open import foundation.propositional-truncation-binary-relations
open import foundation.propositional-truncations
open import foundation.raising-universe-levels
open import foundation.reflecting-maps-equivalence-relations
open import foundation.set-quotients
open import foundation.sets
open import foundation.subtypes
open import foundation.unit-type
open import foundation.universal-property-set-quotients
open import foundation.universe-levels

open import lists.binary-relations-tuples
open import lists.equivalence-relations-finite-sequences
open import lists.equivalence-relations-tuples
open import lists.finite-sequences
open import lists.functoriality-tuples
open import lists.set-quotients-finite-sequences
open import lists.set-quotients-tuples
open import lists.tuples

open import universal-algebra.algebraic-theories
open import universal-algebra.algebras
open import universal-algebra.homomorphisms-of-algebras
open import universal-algebra.models-of-signatures
open import universal-algebra.signatures
open import universal-algebra.terms-over-signatures
open import universal-algebra.universal-property-free-algebras
```

</details>

## Idea

For any [algebraic theory](universal-algebra.algebraic-theories.md) `T` and
generator type `G`, there is an [algebra](universal-algebra.algebras.md) `FG`
satisfying the
[universal property](universal-algebra.universal-property-free-algebras.md) of
free algebras generated by `G`.

This is a canonical construction of the free algebra that takes the type of
words constructed from elements of `G` by operators from the
[signature](universal-algebra.signatures.md) of `T` and constructs the
[set quotient](foundation.set-quotients.md) by an
[equivalence relation](foundation.equivalence-relations.md) formed by the
[equations](universal-algebra.abstract-equations-over-signatures.md) of `T`.

## Definition

```agda
module _
  {l1 l2 l3 : Level}
  (σ : signature l1)
  (T : Algebraic-Theory l2 σ)
  (G : UU l3)
  where

  data word-free-Algebra : UU (l1 ⊔ l3) where
    in-generator-word-free-Algebra : G → word-free-Algebra
    is-model-word-free-Algebra : is-model-of-signature-type σ word-free-Algebra

  data sim-word-free-Algebra : Relation (l1 ⊔ l2 ⊔ l3) word-free-Algebra where
    refl-sim-word-free-Algebra :
      is-reflexive sim-word-free-Algebra
    symmetric-sim-word-free-Algebra :
      is-symmetric sim-word-free-Algebra
    transitive-sim-word-free-Algebra :
      is-transitive sim-word-free-Algebra

    ap-op-sim-word-free-Algebra :
      (op : operation-signature σ)
      (x y : tuple word-free-Algebra (arity-operation-signature σ op)) →
      rel-tuple-Relation
        ( sim-word-free-Algebra)
        ( arity-operation-signature σ op)
        ( x)
        ( y) →
      sim-word-free-Algebra
        ( is-model-word-free-Algebra op x)
        ( is-model-word-free-Algebra op y)

    equation-sim-word-free-Algebra :
      (e : index-Algebraic-Theory σ T) →
      let
        (n , lhs , rhs) = index-abstract-equation-Algebraic-Theory σ T e
      in
        (v : fin-sequence word-free-Algebra n) →
        sim-word-free-Algebra
          ( eval-term σ is-model-word-free-Algebra v lhs)
          ( eval-term σ is-model-word-free-Algebra v rhs)

  trunc-prop-sim-word-free-Algebra :
    Relation-Prop (l1 ⊔ l2 ⊔ l3) word-free-Algebra
  trunc-prop-sim-word-free-Algebra =
    trunc-prop-Relation sim-word-free-Algebra

  rel-trunc-prop-sim-word-free-Algebra :
    Relation (l1 ⊔ l2 ⊔ l3) word-free-Algebra
  rel-trunc-prop-sim-word-free-Algebra =
    type-Relation-Prop trunc-prop-sim-word-free-Algebra

  equivalence-relation-word-free-Algebra :
    equivalence-relation (l1 ⊔ l2 ⊔ l3) word-free-Algebra
  equivalence-relation-word-free-Algebra =
    ( trunc-prop-sim-word-free-Algebra ,
      reflexive-trunc-prop-Relation
        ( sim-word-free-Algebra)
        ( refl-sim-word-free-Algebra) ,
      symmetric-trunc-prop-Relation
        ( sim-word-free-Algebra)
        ( symmetric-sim-word-free-Algebra) ,
      transitive-trunc-prop-Relation
        ( sim-word-free-Algebra)
        ( transitive-sim-word-free-Algebra))

  set-free-Algebra : Set (l1 ⊔ l2 ⊔ l3)
  set-free-Algebra =
    quotient-Set equivalence-relation-word-free-Algebra

  type-free-Algebra : UU (l1 ⊔ l2 ⊔ l3)
  type-free-Algebra = type-Set set-free-Algebra

  in-word-free-Algebra : word-free-Algebra → type-free-Algebra
  in-word-free-Algebra = quotient-map equivalence-relation-word-free-Algebra

  in-generator-free-Algebra : G → type-free-Algebra
  in-generator-free-Algebra g =
    in-word-free-Algebra (in-generator-word-free-Algebra g)

  abstract
    preserves-sim-is-model-word-free-Algebra :
      (op : operation-signature σ) →
      preserves-sim-equivalence-relation
        ( equivalence-relation-tuple
          ( equivalence-relation-word-free-Algebra)
          ( arity-operation-signature σ op))
        ( equivalence-relation-word-free-Algebra)
        ( is-model-word-free-Algebra op)
    preserves-sim-is-model-word-free-Algebra op {xs} {ys} xs|~|ys =
      map-trunc-Prop
        ( ap-op-sim-word-free-Algebra op xs ys)
        ( choice-rel-tuple-trunc-prop-Relation _ _ xs ys xs|~|ys)

  hom-is-model-word-free-Algebra :
    (op : operation-signature σ) →
    hom-equivalence-relation
      ( equivalence-relation-tuple
        ( equivalence-relation-word-free-Algebra)
        ( arity-operation-signature σ op))
      ( equivalence-relation-word-free-Algebra)
  hom-is-model-word-free-Algebra op =
    ( is-model-word-free-Algebra op ,
      preserves-sim-is-model-word-free-Algebra op)

  opaque
    is-model-free-Algebra : is-model-of-signature σ set-free-Algebra
    is-model-free-Algebra op =
      let
        n = arity-operation-signature σ op
      in
        map-is-set-quotient
          ( equivalence-relation-tuple equivalence-relation-word-free-Algebra n)
          ( tuple-Set set-free-Algebra n)
          ( reflecting-map-tuple-quotient-map
            ( equivalence-relation-word-free-Algebra)
            ( n))
          ( equivalence-relation-word-free-Algebra)
          ( set-free-Algebra)
          ( reflecting-map-quotient-map equivalence-relation-word-free-Algebra)
          ( is-set-quotient-tuple-set-quotient
            ( equivalence-relation-word-free-Algebra)
            ( n))
          ( is-set-quotient-set-quotient equivalence-relation-word-free-Algebra)
          ( hom-is-model-word-free-Algebra op)

  model-of-signature-free-Algebra : Model-Of-Signature (l1 ⊔ l2 ⊔ l3) σ
  model-of-signature-free-Algebra =
    ( set-free-Algebra , is-model-free-Algebra)

  abstract opaque
    unfolding is-model-free-Algebra

    compute-is-model-free-Algebra :
      (op : operation-signature σ)
      (v : tuple word-free-Algebra (arity-operation-signature σ op)) →
      is-model-free-Algebra op (map-tuple in-word-free-Algebra v) ＝
      in-word-free-Algebra (is-model-word-free-Algebra op v)
    compute-is-model-free-Algebra op =
      let
        n = arity-operation-signature σ op
      in
        coherence-square-map-is-set-quotient
          ( equivalence-relation-tuple equivalence-relation-word-free-Algebra n)
          ( tuple-Set set-free-Algebra n)
          ( reflecting-map-tuple-quotient-map
            ( equivalence-relation-word-free-Algebra)
            ( n))
          ( equivalence-relation-word-free-Algebra)
          ( set-free-Algebra)
          ( reflecting-map-quotient-map equivalence-relation-word-free-Algebra)
          ( is-set-quotient-tuple-set-quotient
            ( equivalence-relation-word-free-Algebra)
            ( n))
          ( is-set-quotient-set-quotient equivalence-relation-word-free-Algebra)
          ( hom-is-model-word-free-Algebra op)

  abstract
    compute-eval-term-free-Algebra :
      (k : ℕ) (t : term σ k) (v : fin-sequence word-free-Algebra k) →
      eval-term σ is-model-free-Algebra (in-word-free-Algebra ∘ v) t ＝
      in-word-free-Algebra (eval-term σ is-model-word-free-Algebra v t)

    Eq-compute-eval-tuple-term-free-Algebra :
      (k : ℕ) {n : ℕ} (t : tuple (term σ k) n)
      (v : fin-sequence word-free-Algebra k) →
      Eq-tuple
        ( n)
        ( eval-tuple-term σ is-model-free-Algebra (in-word-free-Algebra ∘ v) t)
        ( map-tuple
          ( in-word-free-Algebra)
          ( eval-tuple-term σ is-model-word-free-Algebra v t))

    compute-eval-term-free-Algebra k (var-term i) v = refl
    compute-eval-term-free-Algebra k (op-term op xs) v =
      ( ap
        ( is-model-free-Algebra op)
        ( eq-Eq-tuple _ _ _
          ( Eq-compute-eval-tuple-term-free-Algebra k xs v))) ∙
      ( compute-is-model-free-Algebra
        ( op)
        ( eval-tuple-term σ is-model-word-free-Algebra v xs))

    Eq-compute-eval-tuple-term-free-Algebra k {0} empty-tuple v = map-raise star
    Eq-compute-eval-tuple-term-free-Algebra k {succ-ℕ n} (x ∷ xs) v =
      ( compute-eval-term-free-Algebra k x v ,
        Eq-compute-eval-tuple-term-free-Algebra k xs v)

  abstract
    is-algebra-free-Algebra :
      is-algebra-Model-of-Signature σ T model-of-signature-free-Algebra
    is-algebra-free-Algebra i v =
      let
        (k , lhs , rhs) = index-abstract-equation-Algebraic-Theory σ T i
      in
        ind-is-set-quotient
          ( fin-sequence-equivalence-relation
            ( equivalence-relation-word-free-Algebra)
            ( k))
          ( fin-sequence-Set set-free-Algebra k)
          ( reflecting-quotient-map-fin-sequence
            ( equivalence-relation-word-free-Algebra)
            ( k))
          ( is-set-quotient-fin-sequence-set-quotient
            ( equivalence-relation-word-free-Algebra)
            ( k))
          ( λ v →
            Id-Prop
              ( set-free-Algebra)
              ( eval-term σ is-model-free-Algebra v lhs)
              ( eval-term σ is-model-free-Algebra v rhs))
          ( λ a →
            ( compute-eval-term-free-Algebra k lhs a) ∙
            ( apply-effectiveness-quotient-map'
              ( equivalence-relation-word-free-Algebra)
              ( unit-trunc-Prop (equation-sim-word-free-Algebra i a))) ∙
            ( inv (compute-eval-term-free-Algebra k rhs a)))
          ( v)

  free-Algebra : Algebra (l1 ⊔ l2 ⊔ l3) σ T
  free-Algebra =
    ( model-of-signature-free-Algebra ,
      is-algebra-free-Algebra)
```

## Properties

### The free algebra satisfies the universal property of freely generated algebras

Note that many operations and theorems have separate versions for tuples where
one might expect the [functoriality of tuples](lists.functoriality-tuples.md) to
be used; these are generally necessary to prove to Agda's termination checker
that a chain of recursive calls terminates.

#### Given an algebra `B` and a map `G → B`, the corresponding homomorphism from `FG` to `B`

```agda
module _
  {l1 l2 l3 : Level}
  (σ : signature l1)
  (T : Algebraic-Theory l2 σ)
  (G : UU l3)
  {l4 : Level}
  (B : Algebra l4 σ T)
  (f : G → type-Algebra σ T B)
  where

  map-word-is-free-free-Algebra :
    word-free-Algebra σ T G → type-Algebra σ T B

  map-tuple-word-is-free-free-Algebra :
    {n : ℕ} →
    tuple (word-free-Algebra σ T G) n →
    tuple (type-Algebra σ T B) n

  map-word-is-free-free-Algebra (in-generator-word-free-Algebra x) = f x
  map-word-is-free-free-Algebra (is-model-word-free-Algebra op xs) =
    is-model-set-Algebra σ T B
      ( op)
      ( map-tuple-word-is-free-free-Algebra xs)

  map-tuple-word-is-free-free-Algebra empty-tuple = empty-tuple
  map-tuple-word-is-free-free-Algebra (w ∷ words) =
    map-word-is-free-free-Algebra w ∷
    map-tuple-word-is-free-free-Algebra words

  abstract
    Eq-map-tuple-map-tuple-word-is-free-free-Algebra :
      {n : ℕ} (w : tuple (word-free-Algebra σ T G) n) →
      Eq-tuple
        ( n)
        ( map-tuple-word-is-free-free-Algebra w)
        ( map-tuple map-word-is-free-free-Algebra w)
    Eq-map-tuple-map-tuple-word-is-free-free-Algebra empty-tuple =
      map-raise star
    Eq-map-tuple-map-tuple-word-is-free-free-Algebra (w ∷ words) =
      ( refl ,
        Eq-map-tuple-map-tuple-word-is-free-free-Algebra words)

  abstract
    eval-term-map-word-is-free-free-Algebra :
      {n : ℕ} (t : term σ n)
      (v : fin-sequence (word-free-Algebra σ T G) n) →
      map-word-is-free-free-Algebra
        ( eval-term σ is-model-word-free-Algebra v t) ＝
      eval-term
        ( σ)
        ( is-model-set-Algebra σ T B)
        ( map-word-is-free-free-Algebra ∘ v) t

    eval-tuple-term-map-tuple-word-is-free-free-Algebra :
      {k n : ℕ} (t : tuple (term σ k) n)
      (v : fin-sequence (word-free-Algebra σ T G) k) →
      Eq-tuple
        ( n)
        ( map-tuple-word-is-free-free-Algebra
          ( eval-tuple-term σ is-model-word-free-Algebra v t))
        ( eval-tuple-term
          ( σ)
          ( is-model-set-Algebra σ T B)
          ( map-word-is-free-free-Algebra ∘ v)
          ( t))

    eval-term-map-word-is-free-free-Algebra (var-term x) v =
      refl
    eval-term-map-word-is-free-free-Algebra (op-term op x) v =
      ap
        ( is-model-set-Algebra σ T B op)
        ( eq-Eq-tuple _ _ _
          ( eval-tuple-term-map-tuple-word-is-free-free-Algebra x v))

    eval-tuple-term-map-tuple-word-is-free-free-Algebra empty-tuple v =
      map-raise star
    eval-tuple-term-map-tuple-word-is-free-free-Algebra (x ∷ xs) v =
      ( eval-term-map-word-is-free-free-Algebra x v ,
        eval-tuple-term-map-tuple-word-is-free-free-Algebra xs v)

  abstract
    {-# TERMINATING #-}
    -- TODO: figure out why exactly the termination checker is unhappy
    -- every operation sure looks like it eliminates at least one constructor
    reflects-equivalence-map-word-is-free-free-Algebra' :
      (x y : word-free-Algebra σ T G) →
      sim-word-free-Algebra σ T G x y →
      map-word-is-free-free-Algebra x ＝ map-word-is-free-free-Algebra y

    reflects-equivalence-map-tuple-word-is-free-free-Algebra' :
      {n : ℕ}
      (x y : tuple (word-free-Algebra σ T G) n) →
      rel-tuple-Relation (sim-word-free-Algebra σ T G) n x y →
      Eq-tuple
        ( n)
        ( map-tuple-word-is-free-free-Algebra x)
        ( map-tuple-word-is-free-free-Algebra y)

    reflects-equivalence-map-word-is-free-free-Algebra'
      x .x (refl-sim-word-free-Algebra .x) =
      refl
    reflects-equivalence-map-word-is-free-free-Algebra'
      x y (symmetric-sim-word-free-Algebra .y .x y~x) =
      inv (reflects-equivalence-map-word-is-free-free-Algebra' y x y~x)
    reflects-equivalence-map-word-is-free-free-Algebra'
      x y (transitive-sim-word-free-Algebra .x z .y z~y x~z) =
      ( reflects-equivalence-map-word-is-free-free-Algebra' x z x~z) ∙
      ( reflects-equivalence-map-word-is-free-free-Algebra' z y z~y)
    reflects-equivalence-map-word-is-free-free-Algebra'
      (is-model-word-free-Algebra op x) (is-model-word-free-Algebra .op y)
      (ap-op-sim-word-free-Algebra .op .x .y x~y) =
      ap
        ( is-model-set-Algebra σ T B op)
        ( eq-Eq-tuple _ _ _
          ( reflects-equivalence-map-tuple-word-is-free-free-Algebra' x y x~y))
    reflects-equivalence-map-word-is-free-free-Algebra'
      x y (equation-sim-word-free-Algebra e v) =
      let
        (k , lhs , rhs) = index-abstract-equation-Algebraic-Theory σ T e
      in
        ( eval-term-map-word-is-free-free-Algebra lhs v) ∙
        ( is-algebra-model-Algebra σ T B
          ( e)
          ( map-word-is-free-free-Algebra ∘ v)) ∙
        ( inv (eval-term-map-word-is-free-free-Algebra rhs v))

    reflects-equivalence-map-tuple-word-is-free-free-Algebra'
      empty-tuple empty-tuple _ =
      map-raise star
    reflects-equivalence-map-tuple-word-is-free-free-Algebra'
      (x ∷ xs) (y ∷ ys) (x~y , xs~ys) =
      ( reflects-equivalence-map-word-is-free-free-Algebra' x y x~y ,
        reflects-equivalence-map-tuple-word-is-free-free-Algebra' xs ys xs~ys)

  reflects-equivalence-map-word-is-free-free-Algebra :
    reflects-equivalence-relation
      ( equivalence-relation-word-free-Algebra σ T G)
      ( map-word-is-free-free-Algebra)
  reflects-equivalence-map-word-is-free-free-Algebra {x} {y} =
    rec-trunc-Prop
      ( Id-Prop
        ( set-Algebra σ T B)
        ( map-word-is-free-free-Algebra x)
        ( map-word-is-free-free-Algebra y))
      ( reflects-equivalence-map-word-is-free-free-Algebra' x y)

  reflecting-map-word-is-free-free-Algebra :
    reflecting-map-equivalence-relation
      ( equivalence-relation-word-free-Algebra σ T G)
      ( type-Algebra σ T B)
  reflecting-map-word-is-free-free-Algebra =
    ( map-word-is-free-free-Algebra ,
      reflects-equivalence-map-word-is-free-free-Algebra)

  map-is-free-free-Algebra :
    type-free-Algebra σ T G → type-Algebra σ T B
  map-is-free-free-Algebra =
    inv-precomp-set-quotient
      ( equivalence-relation-word-free-Algebra σ T G)
      ( set-Algebra σ T B)
      ( reflecting-map-word-is-free-free-Algebra)

  abstract
    compute-map-is-free-free-Algebra :
      (w : word-free-Algebra σ T G) →
      map-is-free-free-Algebra (in-word-free-Algebra σ T G w) ＝
      map-word-is-free-free-Algebra w
    compute-map-is-free-free-Algebra =
      is-section-inv-precomp-set-quotient
        ( equivalence-relation-word-free-Algebra σ T G)
        ( set-Algebra σ T B)
        ( reflecting-map-word-is-free-free-Algebra)

    preserves-operations-map-is-free-free-Algebra :
      preserves-operations-Algebra σ T
        ( free-Algebra σ T G)
        ( B)
        ( map-is-free-free-Algebra)
    preserves-operations-map-is-free-free-Algebra op =
      let
        k = arity-operation-signature σ op
      in
        ind-is-set-quotient
          ( equivalence-relation-tuple
            ( equivalence-relation-word-free-Algebra σ T G)
            ( k))
          ( tuple-Set (set-free-Algebra σ T G) k)
          ( reflecting-map-tuple-quotient-map
            ( equivalence-relation-word-free-Algebra σ T G)
            ( k))
          ( is-set-quotient-tuple-set-quotient
            ( equivalence-relation-word-free-Algebra σ T G)
            ( k))
          ( λ v →
            Id-Prop
              ( set-Algebra σ T B)
              ( map-is-free-free-Algebra (is-model-free-Algebra σ T G op v))
              ( is-model-set-Algebra σ T B
                ( op)
                ( map-tuple map-is-free-free-Algebra v)))
          ( λ v →
            equational-reasoning
              map-is-free-free-Algebra
                ( is-model-free-Algebra σ T G
                  ( op)
                  ( map-tuple (in-word-free-Algebra σ T G) v))
              ＝
                map-is-free-free-Algebra
                  ( in-word-free-Algebra σ T G
                    ( is-model-word-free-Algebra op v))
                by
                  ap
                    ( map-is-free-free-Algebra)
                    ( compute-is-model-free-Algebra σ T G op v)
              ＝
                is-model-set-Algebra σ T B
                  ( op)
                  ( map-tuple-word-is-free-free-Algebra v)
                by
                  compute-map-is-free-free-Algebra
                    ( is-model-word-free-Algebra op v)
              ＝
                is-model-set-Algebra σ T B
                  ( op)
                  ( map-tuple map-word-is-free-free-Algebra v)
                by
                  ap
                    ( is-model-set-Algebra σ T B op)
                    ( eq-Eq-tuple _ _ _
                      ( Eq-map-tuple-map-tuple-word-is-free-free-Algebra v))
              ＝
                is-model-set-Algebra σ T B
                  ( op)
                  ( map-tuple
                    ( map-is-free-free-Algebra ∘ in-word-free-Algebra σ T G)
                    ( v))
                by
                  inv
                    ( action-htpy-function
                      ( λ φ → is-model-set-Algebra σ T B op (map-tuple φ v))
                      ( compute-map-is-free-free-Algebra))
              ＝
                is-model-set-Algebra σ T B
                  ( op)
                  ( map-tuple
                    ( map-is-free-free-Algebra)
                    ( map-tuple
                      ( in-word-free-Algebra σ T G)
                      ( v)))
                by
                  ap
                    ( is-model-set-Algebra σ T B op)
                    ( inv
                      ( preserves-comp-map-tuple
                        ( k)
                        ( in-word-free-Algebra σ T G)
                        ( map-is-free-free-Algebra)
                        ( v))))

  hom-is-free-free-Algebra :
    hom-Algebra σ T (free-Algebra σ T G) B
  hom-is-free-free-Algebra =
    ( map-is-free-free-Algebra ,
      preserves-operations-map-is-free-free-Algebra)
```

### The map from `G → B` to homomorphisms `FG → B` is an equivalence

```agda
module _
  {l1 l2 l3 l4 : Level}
  (σ : signature l1)
  (T : Algebraic-Theory l2 σ)
  (G : UU l3)
  (B : Algebra l4 σ T)
  where

  map-inv-is-free-free-Algebra :
    hom-Algebra σ T (free-Algebra σ T G) B → G → type-Algebra σ T B
  map-inv-is-free-free-Algebra (map-φ , _) =
    map-φ ∘ in-generator-free-Algebra σ T G

  abstract
    eq-hom-map-word-is-free-free-Algebra :
      (φ : hom-Algebra σ T (free-Algebra σ T G) B)
      (w : word-free-Algebra σ T G) →
      map-word-is-free-free-Algebra σ T G B
        ( map-hom-Algebra σ T _ B φ ∘ in-generator-free-Algebra σ T G)
        ( w) ＝
      map-hom-Algebra σ T _ B φ (in-word-free-Algebra σ T G w)

    Eq-hom-map-tuple-word-is-free-free-Algebra :
      (φ : hom-Algebra σ T (free-Algebra σ T G) B)
      {n : ℕ} (t : tuple (word-free-Algebra σ T G) n) →
      Eq-tuple
        ( n)
        ( map-tuple-word-is-free-free-Algebra σ T G B
          ( map-hom-Algebra σ T _ B φ ∘ in-generator-free-Algebra σ T G)
          ( t))
        ( map-tuple
          ( map-hom-Algebra σ T _ B φ ∘ in-word-free-Algebra σ T G)
          ( t))

    eq-hom-map-word-is-free-free-Algebra
      φ@(map-φ , preserves-ops-φ) (in-generator-word-free-Algebra x) =
      refl
    eq-hom-map-word-is-free-free-Algebra
      φ@(map-φ , preserves-ops-φ) (is-model-word-free-Algebra op x) =
      equational-reasoning
        is-model-set-Algebra σ T B
          ( op)
          ( map-tuple-word-is-free-free-Algebra σ T G B
            ( map-hom-Algebra σ T _ B φ ∘ in-generator-free-Algebra σ T G)
            ( x))
        ＝
          is-model-set-Algebra σ T B
            ( op)
            ( map-tuple
              ( map-hom-Algebra σ T _ B φ ∘ in-word-free-Algebra σ T G)
              ( x))
          by
            ap
              ( is-model-set-Algebra σ T B op)
              ( eq-Eq-tuple _ _ _
                ( Eq-hom-map-tuple-word-is-free-free-Algebra φ x))
        ＝
          is-model-set-Algebra σ T B
            ( op)
            ( map-tuple
              ( map-hom-Algebra σ T _ B φ)
              ( map-tuple (in-word-free-Algebra σ T G) x))
          by
            ap
              ( is-model-set-Algebra σ T B op)
              ( inv (preserves-comp-map-tuple _ _ _ _))
        ＝
          map-φ
            ( is-model-free-Algebra σ T G
              ( op)
              ( map-tuple (in-word-free-Algebra σ T G) x))
          by inv (preserves-ops-φ op _)
        ＝ map-φ (in-word-free-Algebra σ T G (is-model-word-free-Algebra op x))
          by ap map-φ (compute-is-model-free-Algebra σ T G op x)

    Eq-hom-map-tuple-word-is-free-free-Algebra φ empty-tuple =
      map-raise star
    Eq-hom-map-tuple-word-is-free-free-Algebra φ (w ∷ words) =
      ( eq-hom-map-word-is-free-free-Algebra φ w ,
        Eq-hom-map-tuple-word-is-free-free-Algebra φ words)

    htpy-is-section-map-inv-is-free-free-Algebra :
      (φ : hom-Algebra σ T (free-Algebra σ T G) B) →
      map-is-free-free-Algebra σ T G B (map-inv-is-free-free-Algebra φ) ~
      map-hom-Algebra σ T (free-Algebra σ T G) B φ
    htpy-is-section-map-inv-is-free-free-Algebra φ@(map-φ , preserves-ops-φ) =
      induction-set-quotient
        ( equivalence-relation-word-free-Algebra σ T G)
        ( λ a →
          Id-Prop
            ( set-Algebra σ T B)
            ( map-is-free-free-Algebra σ T G B
              ( map-φ ∘ in-generator-free-Algebra σ T G)
              ( a))
            ( map-φ a))
        ( λ w →
          ( compute-map-is-free-free-Algebra σ T G B
            ( map-φ ∘ in-generator-free-Algebra σ T G)
            ( w)) ∙
          ( eq-hom-map-word-is-free-free-Algebra φ w))

    htpy-is-retraction-map-inv-is-free-free-Algebra :
      (f : G → type-Algebra σ T B) →
      map-inv-is-free-free-Algebra (hom-is-free-free-Algebra σ T G B f) ~ f
    htpy-is-retraction-map-inv-is-free-free-Algebra f g =
      compute-map-is-free-free-Algebra σ T G B f _

  is-free-free-Algebra :
    is-equiv map-inv-is-free-free-Algebra
  is-free-free-Algebra =
    is-equiv-is-invertible
      ( hom-is-free-free-Algebra σ T G B)
      ( λ f →
        eq-htpy (htpy-is-retraction-map-inv-is-free-free-Algebra f))
      ( λ φ →
        eq-type-subtype
          ( preserves-operations-prop-Algebra σ T (free-Algebra σ T G) B)
          ( eq-htpy
            ( htpy-is-section-map-inv-is-free-free-Algebra φ)))
```
