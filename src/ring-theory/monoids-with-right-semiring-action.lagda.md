# Monoids with right semiring action

```agda
module ring-theory.monoids-with-right-semiring-action where
```
...
A {{#concept "monoid with a right semiring action" Agda=Monoid-With-Right-Semiring-Action}}
from $R$ consists of a [monoid](group-theory.monoids.md) $M$ and a binary operation $\mu : M \to R \to M$ satisfying the axioms of a ring action:
...
module _
  ...
  associative-mul-Monoid-With-Right-Semiring-Action :
    ...
    mul-Monoid-With-Right-Semiring-Action
      ...
      ( z) ＝
    mul-Monoid-With-Right-Semiring-Action x
...
module _
  ...
  additive-monoid-with-right-semiring-action-Semiring =
    ( additive-monoid-Semiring R ,
      ...
      inv-associative-mul-Semiring R ,
