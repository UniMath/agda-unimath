...
module _
  ...
  abstract
    ...
    left-distributive-mul-sum-finite-Semiring A@(_ , is-finite-A) x f =
      rec-trunc-Prop
        ( Id-Prop (set-Semiring R) _ _)
        ( λ cA →
          ( ap
            ( mul-Semiring R x)
          ...
          ( inv (eq-product-finite-product-count-Commutative-Monoid _ A cA _)))
        ( is-finite-A)
    ...
    right-distributive-mul-sum-finite-Semiring :
      ...
      sum-finite-Semiring R A (mul-Semiring' R x ∘ f)
    right-distributive-mul-sum-finite-Semiring A@(_ , is-finite-A) f x =
      rec-trunc-Prop
        ( Id-Prop (set-Semiring R) _ _)
        ( λ cA →
          ( ap
