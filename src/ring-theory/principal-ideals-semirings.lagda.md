...
An [ideal](ring-theory.semirings.md) $I$ of a
[semiring](ring-theory.semirings.md) $R$ is said to be a
{{#concept "principal ideal" Disambiguation="semiring" Agda=is-principal-ideal-Semiring}}
if there exists an element $a \in I$ so that every element $I$ is a linear
combination of $a$s {{#cite golan1999}}. In other words, the ideal $I$ is
principal if there exists an element $a$ such that for every element $b$ the
...
The principal left ideal generated by an element $a$ consists of all element of
the form $xa$.
