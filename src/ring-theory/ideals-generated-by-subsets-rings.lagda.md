...
If `S` is a [subset](ring-theory.subsets-rings.md) of a [ring](ring-theory.rings.md) `R`, then the **(two-sided) ideal generated
by `S`** is the least [ideal](ring-theory.ideals-rings.md) containing `S`. This idea
...
`S`. The operation `S ↦ (S)` which maps a subset `S` to the ideal generated by `S` is the lower adjoint of a [galois connection](order-theory.galois-connections-large-posets.md) between the [large poset](order-theory.large-posets.md) of subsets of `R` and the large poset of ideals of `R`.
...
#### The operation `S ↦ (S)` is order preserving
