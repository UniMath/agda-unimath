# The cosine power series on the real numbers

```agda
{-# OPTIONS --lossy-unification #-}

module analysis.cosine-power-series-real-numbers where
```
...
The [cosine function](analysis.cosine-function-real-numbers.md) is defined by
the power series

$$
∑_{n=0}^∞ (-1)^{2n} \frac{x^{2n}}{(2n)!}
$$
...
```agda
coefficient-cos-power-series-at-zero-ℝ : (l : Level) → sequence (ℝ l)
coefficient-cos-power-series-at-zero-ℝ l n =
  let
    case : ℤ-Mod 4 → ℝ l
    case =
      λ {
        (inr star) → raise-ℝ l zero-ℝ ;
        (inl (inr star)) →
          neg-ℝ
            ( raise-real-ℚ l (reciprocal-rational-ℕ⁺ (nonzero-factorial-ℕ n))) ;
        (inl (inl (inr star))) → raise-ℝ l zero-ℝ ;
...
cos-power-series-at-zero-ℝ : (l : Level) → power-series-at-zero-ℝ l
cos-power-series-at-zero-ℝ l =
  power-series-at-zero-coefficients-ℝ (coefficient-cos-power-series-at-zero-ℝ l)

term-at-point-cos-power-series-at-zero-ℝ : {l : Level} → ℝ l → sequence (ℝ l)
term-at-point-cos-power-series-at-zero-ℝ {l} =
  term-at-point-power-series-at-zero-ℝ (cos-power-series-at-zero-ℝ l)
```

## Properties

### The cosine power series absolutely converges everywhere

```agda
abstract
  leq-abs-term-at-point-cos-exp-power-series-at-zero-ℝ :
    {l : Level} (x : ℝ l) (n : ℕ) →
    leq-ℝ
      ( abs-ℝ (term-at-point-cos-power-series-at-zero-ℝ x n))
      ( abs-ℝ (term-at-point-exp-power-series-at-zero-ℝ x n))
  leq-abs-term-at-point-cos-exp-power-series-at-zero-ℝ {l} x n
    with mod-ℕ 4 n
  ... | inr star =
  ...
  is-absolutely-convergent-everywhere-cos-power-series-at-zero-ℝ :
    {l : Level} (x : ℝ l) →
    is-absolutely-convergent-at-point-power-series-at-zero-ℝ
      ( cos-power-series-at-zero-ℝ l)
      ( x)
  is-absolutely-convergent-everywhere-cos-power-series-at-zero-ℝ {l} x =
    is-convergent-is-nonnegative-is-bounded-by-convergent-series-ℝ
      ...
      ( leq-abs-term-at-point-cos-exp-power-series-at-zero-ℝ x)

  is-convergent-everywhere-cos-power-series-at-zero-ℝ :
    (l : Level) →
    is-convergent-everywhere-power-series-at-zero-ℝ
      ( cos-power-series-at-zero-ℝ l)
  is-convergent-everywhere-cos-power-series-at-zero-ℝ l x =
    is-convergent-is-absolutely-convergent-series-ℝ
      ( compute-series-at-point-power-series-at-zero-ℝ
        ( cos-power-series-at-zero-ℝ l)
        ( x))
      ( is-absolutely-convergent-everywhere-cos-power-series-at-zero-ℝ x)
```

### The convergent power series of the cosine function

```agda
cos-convergent-everywhere-power-series-ℝ :
  (l : Level) → convergent-everywhere-power-series-at-zero-ℝ l
cos-convergent-everywhere-power-series-ℝ l =
  ( cos-power-series-at-zero-ℝ l ,
    is-convergent-everywhere-cos-power-series-at-zero-ℝ l)
```
