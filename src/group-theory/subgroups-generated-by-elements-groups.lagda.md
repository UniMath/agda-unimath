...
An alternative definition of the subgroup generated by one element `g` is as the image of the [initial group homomorphism](free-groups-with-one-generator.md) `f : ℤ → G` that satisfies `f(1) ＝ g`.
...
module _
  ...
  is-closed-undere-eq-image-hom-element-Group' :
...
module _
  ...
  contains-powers-subgroup-element-Group :
    (k : ℤ) → is-in-subgroup-element-Group G g (power-int-Group G k g)
