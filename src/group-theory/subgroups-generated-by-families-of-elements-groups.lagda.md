...
### Given a family of elements `x : I → G` and a map `f : J → I`, the subgroup `⟨x ∘ f⟩` is contained in the subgroup `⟨x⟩`

```agda
module _
  {l1 l2 l3 : Level}
  (G : Group l1) {I : UU l2} {J : UU l3} (x : I → type-Group G) (f : J → I)
  where

  contains-precomp-subgroup-family-of-elements-Group :
    leq-Subgroup G
      ( subgroup-family-of-elements-Group G (x ∘ f))
      ( subgroup-family-of-elements-Group G x)
  contains-precomp-subgroup-family-of-elements-Group =
    preserves-order-subgroup-subset-Group G
      ( subtype-im (x ∘ f))
      ( subtype-im x)
...
module _
  {l1 l2 l3 : Level}
  (G : Group l1) {I : UU l2} {J : UU l3}
  (y : J → type-Group G) (x : I → type-Group G) (f : J → I)
  (H : coherence-triangle-maps y x f)
  where

  inclusion-coherence-triangle-family-of-elements-Group :
    leq-Subgroup G
      ( subgroup-family-of-elements-Group G y)
      ( subgroup-family-of-elements-Group G x)
  inclusion-coherence-triangle-family-of-elements-Group =
    preserves-order-subgroup-subset-Group G
      ( subtype-im y)
