...
The operation `S ↦ (S)` mapping a subset of a group to the subgroup generated by
`S` is a [closure operation](order-theory.closure-operations-large-locales.md)
on the [large locale](order-theory.large-locales.md)
of subsets of `G`. Its closed elements are the subgroups of `G`. Since
intersections of subgroups are again subgroups, and since furthermore the equation

```text
 H ∩ (S) ＝ (H ∩ S)
```

holds for any subgroup `H ≤ G` and any subset `S` of `G`, it follows that the
[large poset](order-theory.large-posets.md) of subgroups of a group forms a
large locale. The fact that the subgroups form a locale is recorded in
[`large-locale-of-subgroups-groups`](group-theory.large-locale-of-subgroups-groups.md).
...
#### Formal combinatios of elements in `S`
...
### For any subgroup `H` and any subset `S` of `G` we have `H ∩ (S) ＝ (H ∩ S)`.

```agda
module _
  {l1 l2 l3 : Level} (G : Group l1) (H : Subgroup l2 G) (S : subset-Group l3 G)
  where

  forward-inclusion-is-uniforml-closure-operation-subgroup-subset-Group :
    subset-intersection-Subgroup G H (subgroup-subset-Group G S) ⊆
    subset-subgroup-subset-Group G
      ( intersection-subtype (subset-Subgroup G H) S)
  forward-inclusion-is-uniforml-closure-operation-subgroup-subset-Group x (p , q) =
    apply-universal-property-trunc-Prop q
      {! is-in-subgroup-subset-Group G!}
      {!!}

  is-uniform-closure-operation-subgroup-subset-Group :
    intersection-Subgroup G H (subgroup-subset-Group G S) ＝
    subgroup-subset-Group G (intersection-subtype (subset-Subgroup G H) S)
  is-uniform-closure-operation-subgroup-subset-Group =
