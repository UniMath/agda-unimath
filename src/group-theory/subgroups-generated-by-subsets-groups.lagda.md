...
finite combinations of elements in `S` and of inverses of elements in `S`.

The operation `S ↦ (S)` mapping a subset of a group to the subgroup generated by `S` is a closure operation on the [large locale](order-theory.large-locales.md) of subsets of `G`. Its closed elements are the subgroups of `G`. Since intersections of subgroups are again subgroups, it follows that the [large poset](order-theory.large-posets.md) of subgroups of a group forms a large locale. The fact that the subgroups form a locale is recorded in [`large-locale-of-subgroups-groups`](group-theory.large-locale-of-subgroups-groups).
