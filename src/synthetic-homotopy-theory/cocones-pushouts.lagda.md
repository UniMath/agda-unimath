...
cocone {A = A} {B = B} f g X =
  Σ (A → X) (λ i → Σ (B → X) (λ j → commuting-square g f j i))
...
module _
  ...
  commuting-square-cocone :
    commuting-square g f vertical-map-cocone horizontal-map-cocone
  commuting-square-cocone = pr2 (pr2 c)
```
...
coherence-htpy-cocone f g c c' K L =
  ((commuting-square-cocone f g c) ∙h (L ·r g)) ~
  ((K ·r f) ∙h (commuting-square-cocone f g c'))
...
pr2 (pr2 (cocone-map f g c h)) = h ·l commuting-square-cocone f g c
...
cocone-map-id f g c =
  eq-pair-Σ refl
    ( eq-pair-Σ refl (eq-htpy (λ s → ap-id (commuting-square-cocone f g c s))))
...
pr2 (pr2 (cocone-compose-horizontal f i k c d)) =
  ( ( horizontal-map-cocone (vertical-map-cocone f i c) k d) ·l
    ( commuting-square-cocone f i c)) ∙h
  ( commuting-square-cocone (vertical-map-cocone f i c) k d ·r i)
```
