...
all-sim-Eq-Relation :
  ...
  ( R : (i : Fin n) → Eq-Relation l2 (A i)) →
  ( Eq-Relation l2 (multivariable-input n A))
all-sim-Eq-Relation {l1} {l2} zero-ℕ A R =
  raise-indiscrete-Eq-Relation l2 (raise-unit l1)
all-sim-Eq-Relation (succ-ℕ n) A R =
  prod-Eq-Relation (R (inr star))
    ( all-sim-Eq-Relation n
...
reflects-quotient-vector-map :
  ...
  ( A : functional-vec (UU l1) n)
  ( R : (i : Fin n) → Eq-Relation l2 (A i)) →
  reflects-Eq-Relation (all-sim-Eq-Relation n A R) (quotient-vector-map n A R)
...
equiv-set-quotient-vector :
  ...
  ( A : functional-vec (UU l1) n)
  ( R : (i : Fin n) → Eq-Relation l2 (A i)) →
  set-quotient-vector n A R ≃ set-quotient (all-sim-Eq-Relation n A R)
  ...
  equivalence-reasoning
    ...
    ≃ set-quotient (R (inr star)) ×
        ( set-quotient
          ...
          ( λ x → R (inl x))))
      by lemma
    ≃ set-quotient (all-sim-Eq-Relation (succ-ℕ n) A R)
...
map-equiv-equiv-set-quotient-vector-quotient-map :
  ...
  ( map-equiv (equiv-set-quotient-vector n A R) ∘
    ( quotient-vector-map n A R)) ~
  ( quotient-map (all-sim-Eq-Relation n A R))
...
map-equiv-equiv-set-quotient-vector-quotient-map (succ-ℕ n) A R (a0 , a) =
  ...
  ( triangle-uniqueness-prod-set-quotient
    ( R (inr star))
    ( all-sim-Eq-Relation n (λ z → tail-functional-vec n A z)
    ( λ i → R (inl i)))
    ( a0 , a))
...
inv-precomp-vector-set-quotient :
  ...
  ( R : (i : Fin n) → Eq-Relation l2 (A i)) →
  (X : Set l) →
  reflecting-map-Eq-Relation (all-sim-Eq-Relation n A R) (type-Set X) →
...
inv-precomp-vector-set-quotient (succ-ℕ n) A R X f (qa0 , qa) =
  inv-precomp-set-quotient-prod-set-quotient
    ( R (inr star))
    ( all-sim-Eq-Relation n (tail-functional-vec n A) (λ x → R (inl x)))
    ( X)
    ( f)
  ...
  eq-pair-Σ
    ...
    ( eq-is-prop
      ( is-prop-reflects-Eq-Relation
        ( raise-indiscrete-Eq-Relation l2 (raise-unit l1))
        ( X)
        ( map-reflecting-map-Eq-Relation _ f)))
  ...
  eq-pair-Σ
    ( eq-htpy
      ( λ (a0 , a) →
        ...
        ( htpy-eq
          ( ap
            ( map-reflecting-map-Eq-Relation _)
...
pr2 (is-retraction-inv-precomp-vector-set-quotient (succ-ℕ n) A R X) f =
  ...
  is-inv-map-inv-equiv-f =
    ap
      ( inv-precomp-set-quotient-prod-set-quotient
        ( R (inr star))
        ( all-sim-Eq-Relation n (tail-functional-vec n A)
        ( λ x → R (inl x)))
        ( X))
      ( eq-pair-Σ
