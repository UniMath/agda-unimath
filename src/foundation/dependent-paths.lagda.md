...
module _
  ...
  tr-path-over-eq-inv-tr²-concat :
    (tr (λ t → path-over B t b0 b1) α) ＝ (inv (tr² B α b0) ∙_)
  tr-path-over-eq-inv-tr²-concat =
    map-inv-equiv
      ( htpy-eq ,
        funext (tr (λ t → path-over B t b0 b1) α) (inv (tr² B α b0) ∙_))
      ( tr-path-over)

  tr-inv-path-over-eq-tr²-concat :
    (tr (λ t → path-over B t b0 b1) (inv α)) ＝ ((tr² B α b0) ∙_)
  tr-inv-path-over-eq-tr²-concat =
    map-inv-equiv
      ( htpy-eq ,
        funext (tr (λ t → path-over B t b0 b1) (inv α)) (tr² B α b0 ∙_))
      ( tr-inv-path-over)
...
module _
  ...
  d-right-unit :
    ...
    path-over²
      ( B)
      ( right-unit {p = p})
      ( d-concat B p q refl (refl-path-over B))
      ( q)
  ...
  d-right-unit' :
    ...
    ( tr
      ...
      ( right-unit)
      (d-concat B p q refl (refl-path-over B))) ＝ q
  d-right-unit' p q = tr-path-over-path-over² B (right-unit {p = p})
    (d-concat B p q refl (refl-path-over B)) q (d-right-unit p q)
  ...
  d-left-unit :
    ...
    path-over²
      ( B)
      ( left-unit {p = p})
      ( d-concat B refl (refl-path-over B) p q)
      ( q)
  ...
  d-left-unit' p q = tr-path-over-path-over² B (left-unit {p = p})
    (d-concat B refl (refl-path-over B) p q) q (d-left-unit p q)
