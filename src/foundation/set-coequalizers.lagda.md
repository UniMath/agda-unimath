...
some `z : A` with `f(z) = x` and `g(z) = y`. This quotient `B/A` has the
universal property than any map `h : B → C` for `C` a set coequalizing `f` and
`g` must factor uniquely through the quotient map `B → B/A`.
